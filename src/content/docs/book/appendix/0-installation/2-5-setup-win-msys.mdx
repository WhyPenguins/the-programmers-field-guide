...
<div class="not-content">
  <YouTube id="Hayjgo1Kqg0" params="fs=1&modestbranding=1&rel=0" posterQuality="max"/>
</div>
:::
...
4. **Add MINGW64 Terminal to Visual Studio Code** (important step!):

    To be able to use the MSYS2/MINGW64 terminal as an integrated terminal in Visual Studio Code, follow the steps in the [Set up VS Code Terminal](#set-up-vs-code-terminal) section below.

5. Lastly, try opening a "New Terminal" in VS Code (*Terminal* > *New Terminal*). You should now see the MINGW64 terminal integrated in VS Code.

6. Then, once you're finished, you can go to the [Using MSYS2](#using-msys2) section for more information about accessing files in your C drive with the MSYS2/MINGW64 terminal.
...
<div class="not-content">
  <YouTube id="XxaHFoGdlj0" params="fs=1&modestbranding=1&rel=0" posterQuality="max"/>
</div>
...
<div class="not-content">
  <YouTube id="ndP8vrf9oOU" params="fs=1&modestbranding=1&rel=0" posterQuality="max"/>
</div>
:::
...
1. Copy and paste the following command into your **MINGW64** terminal to download and run the SplashKit installer:
    ...
    If you have issues installing the SplashKit SDK, go to [this troubleshooting page](https://splashkit.io/troubleshoot/windows-msys2/issue-1-bash-curl-not-working/) on the SplashKit website for an alternative installation process.
    :::
...
<div class="not-content">
  <YouTube id="UqGIPYWprRE" params="fs=1&modestbranding=1&rel=0" posterQuality="max"/>
</div>
:::
...
5. Create a new line just before the last closing curly brace `}` at the end of the file, and then copy and paste the text below into that line:
    ...
    - If you already have some settings included in this file, you will need to add a comma `,` after the last line before the closing curly brace `}`, then press Enter and paste the code shown above.
    ...
    - The `"terminal.integrated.defaultProfile.windows": "MSYS2",` line of the code above will set your default profile, so that it will automatically use the MSYS2 (MINGW64) terminal. You can also read this [article](https://code.visualstudio.com/docs/terminal/profiles) to see other ways to set the default terminal.
...
8. Run the following command to enable the integrated MINGW64 terminal in VS Code to update the `.bash_history` file:
    ...
    :::note[Using previous commands]

    When you run a command in the terminal, you can press the *up arrow* key to scroll through your previous commands. These previous commands are stored in the `.bash_history` file.

    By default, terminal commands run using the integrated MINGW64 terminal in VS Code are not added to the `.bash_history` file. The command above fixes this issue.
...
<div class="not-content">
  <YouTube id="lmJjNbPwAlw" params="fs=1&modestbranding=1&rel=0" posterQuality="max"/>
</div>
:::
...
<div class="not-content">
  <YouTube id="s2YTKsRjkWI" params="fs=1&modestbranding=1&rel=0" posterQuality="max"/>
</div>
:::
