...
<div class="not-content">
  <YouTube id="xtxxKzOQFrs" params="fs=1&modestbranding=1&rel=0" posterQuality="max"/>
</div>
...
3. Perform the actions from the current line of code:
    ...
    - With a method call, you will need to know what the procedure does. For example, if the code calls `write_line` you can indicate what is shown in the Terminal without having to step through the instructions in `write_line`.
...
In some of your programs you may have variables that go out of scope. In these cases it is important to illustrate this by crossing out the variable itself. The following image illustrates this with an adjusted version of the same code. Here the `i` variable is declared within the for loop. This means that the `i` variable is destroyed when the loop ends. Notice how in this case I have crossed out the while `i` variable.

![Example of hand execution for a variable that goes out of scope](./images/hand-execution-scope-updated.png)

:::tip
Remember variables exist within a given [scope](/book/part-1-instructions/3-control-flow/5-reference/02-1-scope). When the variable goes out of scope it is destroyed, so you cannot access it any longer.
:::
