...
Sometimes you get part-way through a loop and know that the remainder of the loop's code is not needed, and there is no need to check the loop's condition again. If you can capture that in a condition, then combining a [branching statement](/book/part-1-instructions/3-control-flow/5-reference/03-0-branching) with a `break` will let you jump out of the loop.
...
int main()
{
    write_line("Before you stands a 12 foot tall Knight...");
    write_line();
    write_line("\"We are the Knights who say 'ni'.\"");
    write_line("\"I will say Ni to you again if you do not appease us!\"");
    ...
    write_line("\"Bring us a Shrubbery!\"");
}
```
...
<div class="not-content">
  <YouTube id="zIV4poUZAQo" params="fs=1&modestbranding=1&rel=0" posterQuality="max"/>
</div>

:::note[Summary]

- The break statement is an **instruction** allowing you to jump to the end of the current loop.
- The break statement is typically coded within a [branching](/book/part-1-instructions/3-control-flow/5-reference/03-0-branching) statement that checks if the loop should terminate early.
