...
Looking back to the chapter, remember that computers are electronic devices and that everything else on top of that is an **abstraction** that we build. So, how do the tools we have allow us to use different kinds of data within our programs?
...
As we learned in [literals](/book/part-1-instructions/1-sequence/5-reference/14-literal), a literal string value is enclosed in double quotes (`"`), and a character value is enclosed in single quotes (`'`).
...
One factor to consider when you are using real numbers is that the way they are stored in memory means that you can only represent close approximations of any fractional value.
...
| `to_integer("81")`          | 81                                      |  int     |
| `to_double("27.5")`         | 27.5                                    |  double  |
| `(int)27.5` | 27                                     |  int  |
...
<div class="not-content">
  <YouTube id="24tac93iRLo" params="fs=1&modestbranding=1&rel=0" posterQuality="max"/>
</div>
</div>
